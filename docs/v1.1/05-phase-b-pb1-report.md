...
| --- | --- |
| PB-1 代码 + 0014 + 冒烟 | 🟢 完成 |
| **dry-run 全矩阵实测**（Gate Q5：Upload→409/422→Retry→Delete→Lease→Sweeper→HEAD→ZIP） | ⏸ **需 Owner**：`wrangler secret put GITHUB_TOKEN` + vars 指向独立 private 演练仓库 + 部署授权 |
| Stage 1（生产 1 张图迁移） | 🚫 PB-1 验收 + 单独授权后才启动 |
| Stage 2 | 🚫 更后置授权 |
